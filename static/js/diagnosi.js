...
async function detectGatewayTracerouteUI() {
    ...
    if (!target) {
        alert(tr('diagEnterAClientOr'));
        return;
    }
    const input = document.getElementById('diagGatewayInput');
    if (input) input.placeholder = tr('diagDetectingViaTraceroute');
    try {
        ...
        if (res && res.ok) {
            ...
            if (data.known && data.gateway_ip) {
                ...
                alert(tr('diagGatewayDetectedViaTraceroute', {gateway_ip: data.gateway_ip}));
            } else {
                alert(data.reason || (tr('diagNoGatewayHopDetected')));
            }
        }
    } catch (e) {
...
async function runDiagnosi() {
    ...
    if (!host) return;
    if (!client.trim()) {
        host.innerHTML = _diagEmpty(tr('diagEnterAnIpOr'));
        return;
    }
    ...
    host.innerHTML = `<div class="panel" style="padding:26px; text-align:center; color:var(--text-muted); font-size:13px;">
        ...
        ${escapeHtml(tr('diagQueryingSwitchAndFirewall'))}
    </div>`;
    ...
    if (!res || !res.ok) {
        ...
        host.innerHTML = _diagEmpty(escapeHtml(e.detail || (tr('diagDiagnosisFailed'))), true);
        return;
    }
...
function _diagCard(icon, title, section, bodyFn, extra) {
    ...
    if (err) {
        ...
        body = `<div style="color:var(--text-muted); font-size:12px;"><i class="fa-solid fa-circle-info" style="margin-right:6px;"></i>${escapeHtml((section && section.reason) || (tr('diagNotKnown')))}</div>`;
    } else {
        body = bodyFn(section);
    }
...
function _diagFreshness(f) {
    ...
    return `<div style="display:flex; align-items:center; gap:8px; font-size:11px; color:${color}; margin-bottom:12px;">
        ...
        <span style="font-family:var(--font-code);">
            ${escapeHtml(tr('diagBinding'))} ${escapeHtml(_diagAge(ages.binding_s))} ·
            ${escapeHtml(tr('diagPort'))} ${escapeHtml(_diagAge(ages.port_s))}
        </span>
        ${(f.scanned || []).length ? `<span>${escapeHtml((tr('diagReScanned')) + f.scanned.join(', '))}</span>` : ''}
    </div>`;
}
...
function renderDiagnosi(d, dest) {
    ...
    const badge = d.complete
        ? `<span class="badge" style="color:var(--success);">${escapeHtml(tr('diagComplete'))}</span>`
        : `<span class="badge" style="color:var(--warning);" title="${escapeHtml(tr('diagSomeSectionsCouldNot'))}">${escapeHtml(tr('diagPartial'))}</span>`;
    ...
    // hardware reale.
    const previewBadge = `<span class="preview-badge" title="${escapeHtml(tr('diagNotYetValidatedAgainst'))}">preview</span>`;
    ...
    const position = _diagCard('fa-location-dot', tr('diagPosition'), s.position, p =>
        ...
        (p.l2_only ? `<div style="color:var(--warning); font-size:12px; margin-top:6px;">
            ...
            <b>${escapeHtml(tr('diagLOnly'))}</b> — ${escapeHtml(p.binding_reason || '')}</div>` : '') +
        (p.ambiguous ? `<div style="color:var(--warning); font-size:12px; margin-top:6px;"><i class="fa-solid fa-triangle-exclamation" style="margin-right:6px;"></i>${escapeHtml(tr('diagOtherBindingsForThis'))}: ${p.ambiguous.map(a => escapeHtml(a.mac)).join(', ')}</div>` : '')
    );

    const l2 = _diagCard('fa-ethernet', tr('diagLLinkHealth'), s.l2_health, h => {
        const i = h.interface || {};
        let out = '';
        if (i.known) {
            const d1 = i.error_delta || {};
            out += _kv('Link', `${i.link || '—'} (admin ${i.admin_status || '—'})`);
            out += _kv(tr('diagErrorsInOut'),
                i.error_delta === null
                    ? (tr('diagSingleSampleCannotTell'))
                    : `${d1.in_errors === null ? '?' : d1.in_errors} / ${d1.out_errors === null ? '?' : d1.out_errors}` +
                      (i.error_window_s ? ` (${i.error_window_s}s)` : ''));
            if (i.erroring) out += `<div style="color:var(--danger); font-size:12px; margin-top:6px;"><i class="fa-solid fa-triangle-exclamation" style="margin-right:6px;"></i>${escapeHtml(tr('diagErrorCountersAreRising'))}</div>`;
        } else {
        ...
        if (h.vlan_mismatch) out += `<div style="color:var(--warning); font-size:12px; margin-top:6px;">
            <i class="fa-solid fa-triangle-exclamation" style="margin-right:6px;"></i>${escapeHtml(tr('diagPortMovedVlan'))}:
    ...
    const history = _diagCard('fa-clock-rotate-left', tr('diagHistory'), s.history, hs => {
        if (hs.empty) return `<div style="color:var(--text-muted); font-size:12px;">${escapeHtml(tr('diagNeverSeenBeforeThis'))}</div>`;
        ...
        return `<div style="font-size:12px; font-weight:600; margin-bottom:4px;">${escapeHtml(tr('diagPositions'))}</div>${pos || '—'}
            <div style="font-size:12px; font-weight:600; margin:10px 0 4px;">${escapeHtml(tr('diagAddresses'))}</div>${ips || '—'}
            <div style="color:var(--text-muted); font-size:11px; margin-top:8px;">${escapeHtml(tr('diagRetention'))}: ${escapeHtml(hs.retention_days)} ${escapeHtml(tr('diagDays'))}. ${escapeHtml(tr('diagRowsAggregateRepeatedMoves'))}</div>`;
    });

    const path = _diagCard('fa-route', tr('diagTrafficPath'), s.path, p =>
    ...
    const relayNote = relay ? `<div style="margin-top:8px; padding-top:8px; border-top:1px solid var(--border);">
        ${relay.known
            ? `<div style="font-size:12px; font-weight:600; margin-bottom:4px;">${escapeHtml(tr('diagMatchingPolicyViaRelay'))}</div>
    ...
    const fw = _diagCard('fa-shield-halved', 'Firewall', s.firewall, f => {
        let out = _kv('FortiGate', f.fortigate) + _kv(tr('diagChosenBy'), f.resolved_by);
        const sub = f.sections || {};
        const pl = sub.policy_lookup;
        if (pl && pl.data) {
            out += `<div style="margin-top:8px; padding-top:8px; border-top:1px solid var(--border); font-size:12px; font-weight:600;">${escapeHtml(tr('diagMatchingPolicy'))}</div>`;
        ...
        if (failed.length) out += `<div style="color:var(--text-muted); font-size:11px; margin-top:8px;">${escapeHtml(tr('diagSectionsUnavailable'))}: ${escapeHtml(failed.join(', '))}</div>`;
        out += `<details style="margin-top:8px;"><summary style="cursor:pointer; font-size:11px; color:var(--text-muted);">${escapeHtml(tr('diagRawFortigateOutput'))}</summary>
    ...
    const across = !s.across_sites ? '' : _diagCard('fa-tower-broadcast',
        ...
        if (a.route) {
            ...
            out += _kv(tr('diagRouteToDestination'),
                a.route.error ? a.route.error
                    : (rd.matched ? `${rd.destination} → ${rd.gateway || '—'} (${rd.interface || '—'})`
                                  : (tr('diagNoRoute'))));
            if (rd.matched === false) out += `<div style="color:var(--danger); font-size:12px; margin-top:6px;"><i class="fa-solid fa-triangle-exclamation" style="margin-right:6px;"></i>${escapeHtml(tr('diagTheFirewallHasNo'))}</div>`;
        }
        return out;
    });

    const denies = _diagCard('fa-ban', tr('diagBlocksLastHour'), s.denies, b => {
        let out = _kv(tr('diagTotal'), b.total);
        ...
        if (b.truncated) out += `<div style="color:var(--warning); font-size:11px; margin-top:6px;">${escapeHtml(tr('diagScanCappedThisIs'))}</div>`;
        return out;
    });
    ...
    host.innerHTML = `
        <div class="panel" style="display:flex; align-items:center; gap:10px; flex-wrap:wrap; margin-bottom:12px; padding:12px 16px;">
            ...
            <button data-action="rerun-diagnosi" class="btn btn-secondary btn-small" style="width:auto; margin:0;"><i class="fa-solid fa-rotate"></i> ${escapeHtml(tr('diagReRun'))}</button>
        </div>
    ...
    const chips = list.map(t => {
        ...
        return `<button data-action="diagnosi-pick-tenant" data-tenant="${escapeHtml(t.tenant)}"
            ...
            <span style="color:var(--text-muted); font-weight:400;">
                ${escapeHtml(t.ip || (tr('diagNoIp')))}${where ? ' · ' + escapeHtml(where) : ''}
        ...
        <div style="display:flex; gap:10px; align-items:flex-start; margin-bottom:14px;">
            ...
            <div>
                <div style="font-size:14px; font-weight:700; margin-bottom:4px;">${escapeHtml(
                    tr('diagThisAddressExistsIn'))}</div>
                <div style="font-size:12px; color:var(--text-muted);">${escapeHtml(
                    tr('diagNoReportWasProduced'))}</div>
            </div>
        </div>
...
function _diagTrunk(t) {
    let out = `<div style="margin-top:10px; padding-top:8px; border-top:1px solid var(--border); font-size:12px; font-weight:600;">
        ${escapeHtml(tr('diagVlanAlongTheTrunk'))}</div>`;
    if (!t.known) {
    ...
    if (t.gateway_source && t.gateway_source !== 'arp') {
        const age = t.gateway_backup_age_s
            ? ` — ${tr('diagBackup')} ${Math.floor(t.gateway_backup_age_s / 86400)}g`
            : '';
        const how = t.gateway_source === 'manual'
            ? (tr('diagDeclaredByAnOperator'))
            : (tr('diagDerivedFromTheConfiguration'));
        out += `<div style="font-size:12px; color:var(--text-muted); margin-top:6px;">
            <i class="fa-solid fa-route" style="margin-right:6px;"></i>${escapeHtml(tr('diagVlanGateway'))}:
            ${escapeHtml(t.gateway_device || '')}${t.gateway_vlan_ip ? ' (' + escapeHtml(t.gateway_vlan_ip) + ')' : ''} — ${how}${age}</div>`;
    }
    if (!t.chain_known) {
        out += `<div style="font-size:12px;">${t.ok
            ? escapeHtml(tr('diagAllowedOnThisSwitch'))
            : `<span style="color:var(--danger);">${escapeHtml(tr('diagMissingOnThisSwitch'))}</span>`}</div>
        ...
        if (t.unreadable_devices && t.unreadable_devices.length) {
            out += `<div style="color:var(--warning); font-size:11px; margin-top:4px;">${escapeHtml(tr('diagSomeDevicesCouldNot'))}${
...
function _diagWifiCard() {
    return `<div class="panel" style="padding:14px 16px; margin-bottom:12px;">
        <div style="display:flex; align-items:center; gap:8px; margin-bottom:10px; font-size:13px; font-weight:600; flex-wrap:wrap;">
            ...
            <i class="fa-solid fa-wifi" style="color:var(--text-muted);"></i> ${escapeHtml(tr('diagWirelessCiscoWlc'))}
            <div style="flex:1; min-width:20px;"></div>
            <input id="diagWlcIp" placeholder="${escapeHtml(tr('diagWlcAddress'))}"
            ...
            <button data-action="diagnose-wifi" class="btn btn-secondary btn-small" style="width:auto; margin:0;"><i class="fa-solid fa-stethoscope"></i> ${escapeHtml(tr('diagRun'))}</button>
        </div>
        <div id="diagWifiBody"><div style="color:var(--text-muted); font-size:12px;">${escapeHtml(
            _diagMac ? (tr('diagClientDetailApWlan'))
                     : (tr('diagNoMacKnownFor')))}</div></div>
    </div>`;
}
...
async function diagnoseWifi() {
    ...
    if (!ip || !_diagMac) {
        host.innerHTML = `<div style="color:var(--text-muted); font-size:12px;">${escapeHtml(
            tr('diagEnterTheWlcAddress'))}</div>`;
        return;
    }
    host.innerHTML = `<div style="color:var(--text-muted); font-size:12px;"><i class="fa-solid fa-circle-notch fa-spin" style="margin-right:8px;"></i>${escapeHtml(tr('diagQueryingTheController'))}</div>`;
    const res = await apiFetch(`/api/wlc/${encodeURIComponent(ip)}/diagnose-client/${encodeURIComponent(_diagMac)}`);
    if (!res || !res.ok) {
        const e = res ? await res.json().catch(() => ({})) : {};
        host.innerHTML = `<div style="color:var(--danger); font-size:12px;">${escapeHtml(e.detail || (tr('diagWifiDiagnosisFailed')))}</div>`;
        return;
    }
    ...
    const rows = Object.keys(sections).map(name => {
        ...
        return _kv(name, sec.error ? (tr('diagError')) + sec.error
                                   : (n === null ? (tr('diagAnswered')) : String(n)));
    }).join('');
    host.innerHTML = _kv('WLC', d.wlc) + _kv(tr('diagPlatform'), d.platform) +
        _kv('MAC', d.client_mac) + rows +
        `<details style="margin-top:8px;"><summary style="cursor:pointer; font-size:11px; color:var(--text-muted);">${escapeHtml(tr('diagRawControllerOutput'))}</summary>
...
function _diagBounceControl() {
    ...
    return `<div style="margin-top:10px; padding-top:8px; border-top:1px solid var(--border);">
        <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
            <span style="font-size:12px; color:var(--text-muted);">${escapeHtml(tr('diagActOnPort'))}
                <b style="font-family:var(--font-code);">${escapeHtml(_diagSwitch.port)}</b></span>
            <input id="diagBounceConfirm" placeholder="${escapeHtml(tr('diagTypeThePortName'))}"
...
async function _diagPortControl(action) {
    ...
    out.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> ${escapeHtml(tr('diagWorking'))}`;
    ...
    if (!res || !res.ok) {
        const e = res ? await res.json().catch(() => ({})) : {};
        out.innerHTML = `<span style="color:var(--danger);">${escapeHtml(e.detail || (tr('diagRefused')))}</span>`;
        return;
    }
    out.innerHTML = action === 'shutdown'
        ? `<span style="color:var(--warning);">${escapeHtml(tr('diagPortIsolatedItStays'))}</span>`
        : `<span style="color:var(--success);">${escapeHtml(tr('diagPortReEnabled'))}</span>`;
}
...
async function diagIsolatePort() {
    ...
    if (typed.toLowerCase() !== String(_diagSwitch.port).toLowerCase()) {
        out.innerHTML = `<span style="color:var(--warning);">${escapeHtml(tr('diagTypeTheExactPort'))}</span>`;
        return;
    }
...
async function diagBouncePort() {
    ...
    out.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> ${escapeHtml(tr('diagBouncing'))}`;
    ...
    if (!res || !res.ok) {
        const e = res ? await res.json().catch(() => ({})) : {};
        out.innerHTML = `<span style="color:var(--danger);">${escapeHtml(e.detail || (tr('diagBounceRefused')))}</span>`;
        return;
    }
    const r = await res.json();
    out.innerHTML = r.up_ok
        ? `<span style="color:var(--success);">${escapeHtml(tr('diagPortBounced'))}</span>`
        : `<span style="color:var(--danger);">${escapeHtml(r.error || (tr('diagPortLeftDown')))}</span>`;
}
